...
import numpy as np
from wrapper.AlgoPython.BitGraph.models.BiaTCGNet.BiaTCGNet import Model
import os
from wrapper.AlgoPython.BitGraph.data.GenerateDataset import loaddataset
import datetime
...
def recovBitGRAPH(ts_m=None, seq_len=24, sliding_windows=1, pred_len=0, kernel_size=1, kernel_set=[2,3,6,7], epochs=10, batch_size=32, subgraph_size=5, num_workers=0, tr_ratio=0.7, seed=0, norma=True, verbose=True, replicat=False):
    ...
    seq_len, kernel_set, kernel_size, subgraph_size, batch_size = utils_imp.patch_classifiers_bitgraph(incomp_data=ts_m, seq_len=seq_len, kernel_set=kernel_set, kernel_size=kernel_size, subgraph_size=subgraph_size, batch_size=batch_size, verbose=verbose)
