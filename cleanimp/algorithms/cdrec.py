...
def cdrec(incomp_data, rank=3, iterations=100, epsilon=0.000001, logs=True, verbose=True, lib_path=None):
    """
    ...
    Example
    -------
        >>> recov_data = cdrec(incomp_data=incomp_data, rank=2, iterations=100, epsilon=0.000001, logs=True)
        >>> print(recov_data)

    """

    rank = utils.control_boundaries(rank=rank, boundary=incomp_data.shape[1])

    if verbose:
        print(f"\n(IMPUTATION) CDRec\n\tMatrix: {incomp_data.shape[0]}, {incomp_data.shape[1]}\n\trank: {rank}\n\tepsilon: {epsilon}\n\titerations: {iterations}\n")
        print(f"call: cdrec.impute(params={{'rank': {rank}, 'epsilon': {epsilon}, 'iterations': {iterations}}})\n")

    start_time = time.time()  # Record start time

    # Call the C++ function to perform recovery
    recov_data = native_cdrec(incomp_data, rank, epsilon, iterations, False)
